...
<body class="dashboard-body">
                                        ...
                                        <div class="application-actions">
                                            <div style="margin-bottom: 10px;">
                                                ...
                                                <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'admin'): ?>
                                                    <div style="background: rgba(255,255,255,0.05); padding: 8px; border-radius: 4px; margin-bottom: 10px; font-size: 11px; font-family: monospace; color: rgba(255,255,255,0.7);">
                                                        <div style="margin-bottom: 4px;"><strong>DEBUG INFO:</strong></div>
                                                        <div>Photo Path in DB: <?php echo htmlspecialchars($app['photo_path'] ?? 'NULL'); ?></div>
                                                        <div>Resume Path in DB: <?php echo htmlspecialchars($app['resume_path'] ?? 'NULL'); ?></div>
                                                        <div>Photo Found: <?php echo $photoExists ? 'YES' : 'NO'; ?></div>
                                                        <div>Resume Found: <?php echo $resumeExists ? 'YES' : 'NO'; ?></div>
                                                        <?php if ($photoExists): ?>
                                                            <div>Photo Actual Path: <?php echo htmlspecialchars($photoPath); ?></div>
                                                        <?php endif; ?>
                                                        <?php if ($resumeExists): ?>
                                                            <div>Resume Actual Path: <?php echo htmlspecialchars($resumePath); ?></div>
                                                        <?php endif; ?>
                                                    </div>
                                                <?php endif; ?>
                                                ...
                                                <div style="display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 10px;">
                                                    <?php 
                                                    // Check if photo exists in the correct directory
                                                    $photoExists = false;
                                                    $photoPath = '';
                                                    if (!empty($app['photo_path'])) {
                                                        ...
                                                        $possiblePaths = [
                                                            $app['photo_path'],
                                                            'uploads/photos/' . basename($app['photo_path']),
                                                            'Admin/uploads/photos/' . basename($app['photo_path']),
                                                            'uploads/photos/' . $app['photo_path'],
                                                            'Admin/uploads/photos/' . $app['photo_path']
                                                        ];
                                                    ...
                                                    if ($photoExists): ?>
                                                        <a href="<?php echo htmlspecialchars($photoPath); ?>" target="_blank" class="btn secondary-btn" style="margin-right: 0;">
                                                            <i class="fas fa-image"></i> View Photo
                                                        </a>
                                                        <button type="button" class="btn info-btn" onclick="previewPhoto('<?php echo htmlspecialchars($photoPath); ?>', '<?php echo htmlspecialchars($app['student_name']); ?>')">
                                                            <i class="fas fa-eye"></i> Preview
                                                        </button>
                                                        <a href="<?php echo htmlspecialchars($photoPath); ?>" download class="btn warning-btn">
                                                            <i class="fas fa-download"></i> Download
                                                        </a>
                                                    <?php else: ?>
                                                        <span class="btn danger-btn" style="cursor: default; opacity: 0.7;">
                                                            <i class="fas fa-times-circle"></i> No Photo
                                                        </span>
                                                    <?php endif; ?>
                                                    
                                                    <?php 
                                                    // Check if resume exists in the correct directory
                                                    $resumeExists = false;
                                                    $resumePath = '';
                                                    if (!empty($app['resume_path'])) {
                                                        ...
                                                        $possiblePaths = [
                                                            $app['resume_path'],
                                                            'uploads/resumes/' . basename($app['resume_path']),
                                                            'Admin/uploads/resumes/' . basename($app['resume_path']),
                                                            'uploads/resumes/' . $app['resume_path'],
                                                            'Admin/uploads/resumes/' . $app['resume_path']
                                                        ];
                                                    ...
                                                    if ($resumeExists): ?>
                                                        <a href="<?php echo htmlspecialchars($resumePath); ?>" target="_blank" class="btn secondary-btn">
                                                            <i class="fas fa-file-pdf"></i> View Resume
                                                        </a>
                                                        <button type="button" class="btn info-btn" onclick="previewResume('<?php echo htmlspecialchars($resumePath); ?>', '<?php echo htmlspecialchars($app['student_name']); ?>')">
                                                            <i class="fas fa-eye"></i> Preview
                                                        </button>
                                                        <a href="<?php echo htmlspecialchars($resumePath); ?>" download class="btn warning-btn">
                                                            <i class="fas fa-download"></i> Download
                                                        </a>
                                                    <?php else: ?>
                                                        <span class="btn danger-btn" style="cursor: default; opacity: 0.7;">
                                                            <i class="fas fa-times-circle"></i> No Resume
                                                        </span>
                                                    <?php endif; ?>
                                                </div>
                                                
                                                <!-- File Information -->
                                                <?php if ($photoExists || $resumeExists): ?>
                                                    <div style="background: rgba(255,255,255,0.1); padding: 8px; border-radius: 4px; margin-bottom: 10px; font-size: 12px;">
                                                        <div style="color: rgba(255,255,255,0.8); margin-bottom: 4px;">
                                                            <i class="fas fa-info-circle"></i> File Information:
                                                        </div>
                                                        <?php if ($photoExists): ?>
                                                            <div style="margin-bottom: 2px;">
                                                                <strong>Photo:</strong> <?php echo basename($photoPath); ?> 
                                                                (<?php echo number_format(filesize($photoPath)) . ' bytes'; ?>)
                                                            </div>
                                                        <?php endif; ?>
                                                        <?php if ($resumeExists): ?>
                                                            <div>
                                                                <strong>Resume:</strong> <?php echo basename($resumePath); ?> 
                                                                (<?php echo number_format(filesize($resumePath)) . ' bytes'; ?>)
                                                            </div>
                                                        <?php endif; ?>
                                                    </div>
                                                <?php endif; ?>
    ...
    <script>
        ...
        function previewPhoto(photoPath, studentName) {
            const modal = document.getElementById('filePreviewModal');
            const title = document.getElementById('previewModalTitle');
            const content = document.getElementById('previewContent');
            
            title.textContent = `Photo Preview - ${studentName}`;
            content.innerHTML = `
                <div style="margin-bottom: 20px;">
                    <img src="${photoPath}" alt="Profile Photo" style="max-width: 100%; max-height: 70vh; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.3);">
                </div>
                <div style="margin-bottom: 20px;">
                    <a href="${photoPath}" target="_blank" class="btn primary-btn" style="margin-right: 10px;">
                        <i class="fas fa-external-link-alt"></i> Open in New Tab
                    </a>
                    <a href="${photoPath}" download class="btn warning-btn">
                        <i class="fas fa-download"></i> Download
                    </a>
                </div>
                <div style="color: rgba(255,255,255,0.7); font-size: 14px;">
                    <strong>File:</strong> ${photoPath.split('/').pop()}<br>
        ...
        function previewResume(resumePath, studentName) {
            const modal = document.getElementById('filePreviewModal');
            const title = document.getElementById('previewModalTitle');
            const content = document.getElementById('previewContent');
            
            title.textContent = `Resume Preview - ${studentName}`;
            content.innerHTML = `
                <div style="margin-bottom: 20px;">
                    <div style="background: rgba(255,255,255,0.1); padding: 20px; border-radius: 8px; border: 2px dashed rgba(255,255,255,0.3);">
                        <i class="fas fa-file-pdf" style="font-size: 4rem; color: #F44336; margin-bottom: 15px;"></i>
                        <h3 style="color: #FFC107; margin-bottom: 10px;">PDF Resume</h3>
                        <p style="color: rgba(255,255,255,0.8); margin-bottom: 20px;">
                            This is a PDF document that cannot be previewed directly in the browser.
                        </p>
                    </div>
                </div>
                <div style="margin-bottom: 20px;">
                    <a href="${resumePath}" target="_blank" class="btn primary-btn" style="margin-right: 10px;">
                        <i class="fas fa-external-link-alt"></i> Open in New Tab
                    </a>
                    <a href="${resumePath}" download class="btn warning-btn">
                        <i class="fas fa-download"></i> Download
                    </a>
                </div>
                <div style="color: rgba(255,255,255,0.7); font-size: 14px;">
                    <strong>File:</strong> ${resumePath.split('/').pop()}<br>
                    <strong>Student:</strong> ${studentName}
                </div>
            `;
